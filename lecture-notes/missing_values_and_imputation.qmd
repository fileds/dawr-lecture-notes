...
penguins_imputed <- penguins |> 
  ...
  mutate(flipper_length_mm = replace_na(flipper_length_mm, mean_flipper_length))
...
sex_model <- penguins |> glm(formula = sex ~ . - island - year - flipper_length_mm, family = binomial)

# Select penguins with missing sex
penguins_missing_sex <- penguins |>
  filter(is.na(sex))

# Predict sex probability
predicted_sex_probability <- predict(sex_model, newdata = penguins_missing_sex, type = "response")
...
#| warning: false
#| code-fold: true
# Plot the distributions of the body weight and color by sex and facet by species
penguins |> 
