...
\hypersetup{
  ...
  pdfauthor={Filip Edström, Joakim Wallmark, and Hugo Morvan},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\author{Filip Edström, Joakim Wallmark, and Hugo Morvan}
\date{2026-09-06}
...
\begin{verbatim}
 ...
 N.unique :3   1st Qu.:23.50   N.unique :3     
 N.blank  :0   Median :25.00   N.blank  :0     
 Min.nchar:3   Mean   :25.67   Min.nchar:3     
 Max.nchar:7   3rd Qu.:27.50   Max.nchar:5     
               Max.   :30.00                   
\end{verbatim}
...
we are going to use the \emph{Akaike Information Criterion} (AIC). AIC
scales with the number of observations, so, in contrast to R\(^2\), the
AIC of a model on its own is not useful. Here we will use it to compare
models and argue for one model over another. Later in this chapter we
will introduce accuracy as a way of measuring the performance of a
...
short section at the end of this chapter that introduces the other types
of missingess and methods to deal with them.
...
\NormalTok{penguins\_imputed }\OtherTok{\textless{}{-}}\NormalTok{ penguins }\SpecialCharTok{|\textgreater{}} 
  \CommentTok{\# Flipper length is integer, so we need to convert it to numeric}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{flipper\_length\_mm =} \FunctionTok{as.numeric}\NormalTok{(flipper\_length\_mm)) }\SpecialCharTok{|\textgreater{}}
...
\section{Other types of missing values and advanced
imputation}\label{other-types-of-missing-values-and-advanced-imputation}
...
missing. For example, if some of the penguins are of poor health the
researchers may choose to not weigh the penguin to avoid the extra
stress of being put in the weigh bag. This results in missing values of
body weight but there is no record of penguin health in the data that
can help us explain the missingness.
...
Lets select only the significant variables (0.05) using backward
selection to have a simpler model. This can be useful if we have more
missing values in other columns, which would prevent us from using these
columns for imputation. Once we have our model with only significant
variables we can use it to predict the sex of the penguins with missing
values.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Fit new model after backward selection}
\NormalTok{sex\_model }\OtherTok{\textless{}{-}}\NormalTok{ penguins }\SpecialCharTok{|\textgreater{}} \FunctionTok{glm}\NormalTok{(}\AttributeTok{formula =}\NormalTok{ sex }\SpecialCharTok{\textasciitilde{}}\NormalTok{ . }\SpecialCharTok{{-}}\NormalTok{ island }\SpecialCharTok{{-}}\NormalTok{ year }\SpecialCharTok{{-}}\NormalTok{ flipper\_length\_mm, }\AttributeTok{family =}\NormalTok{ binomial)}
...
\begin{verbatim}
  ...
  <NA> female   male female female female female female female female   <NA> 
Levels: female male
\end{verbatim}
...
In this chapter we have looked at missing values, types of missingness
mechanisms, and how to impute missing values when values are MCAR and
MAR.
